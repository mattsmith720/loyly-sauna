  ...
  return (
    <section className="local-strip border-y border-[var(--line)] bg-[var(--white-warm)]" aria-label="Service area">
      <div className="wrap py-7 text-center sm:py-8">
        ...
        <StaggerChildren className="local-areas flex flex-wrap items-center justify-center gap-2">
          {suburbs.map((area) => (
            <StaggerItem key={area}>
              <span className="local-chip rounded-full border border-[var(--line)] bg-[var(--cream)] px-3 py-1 text-[0.78rem] font-semibold text-[var(--charcoal-soft)]">
                {area}
              </span>
            </StaggerItem>
          ))}
